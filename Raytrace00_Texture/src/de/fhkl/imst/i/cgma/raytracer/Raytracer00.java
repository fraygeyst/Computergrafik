...
public class Raytracer00 implements IRayTracerImplementation {
	...
	public void setViewParameters(float fovyDegree, float near) {
		...
		resy = gui.getResY();
		this.aspect = (float) resx / resy;


		// set attributes h, w
		this.h = (float) (2* this.near * Math.tan(this.fovyRadians/2));
		this.w = h * this.aspect;


		//Kontrollausgaben
		//System.out.println("aspect: " +this.aspect);
		//System.out.println("fovyRadians: " +this.fovyRadians);
	...
	public void doRayTrace() {
		...
		// set eye point //sitzt im Ursprung
		rayEx = 0.0f;
		rayEy = 0.0f;
		rayEz = 0.0f;
		...
		for (int xp = 0; xp < resx; ++xp) { //läuft also von oben nach unten
			for (int yp = 0; yp < resy; ++yp) {
				...
				y = ( ((this.resy-1) - yp) / (float) (this.resy-1) ) * this.h - ((float) this.h/2) ;


				// ray vector //vom Auge aus zum x,y,z
				rayVx = x;
				rayVy = y;
				rayVz = z;

				//Kontrollausgabe erste Schleife
				/*	if (xp == 0 && yp == 0) {
					System.out.println("x: "+ x);
					System.out.println("y: "+ y);
					System.out.println("z: "+ z);
					//Analog
					//System.out.println("rayVx: "+ rayVx);
	...
	private Color traceRayAndGetColor(float rayEx, float rayEy, float rayEz, float rayVx, float rayVy, float rayVz) { //Strahl in die Szene schiessen
		...
		for (int objectsNumber = 0; objectsNumber < objects.size(); objectsNumber++) {
			...
			if (scene instanceof I_Sphere) {
				...
				// no intersection point? => next object //kein schnittpunkt mit der Kugel oder tuschiert
				if (d <= 0)
					continue;
		...
		l[1] = ICenter[0] - minIP[1];
		l[2] = ICenter[0] - minIP[2];
		normalize(l);
	...
	private float normalize(float[] vec) {
		...
		l = (float) Math.sqrt( (double)  (vec[0] * vec[0]) + (vec[1] * vec[1]) + (vec[2] * vec[2]));
		vec[0]= vec[0]/l;
		vec[1]= vec[1]/l;
		vec[2]= vec[2]/l;
